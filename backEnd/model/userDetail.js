...
const createUser = async (id, email, password, phone_num, first_name, last_name, admin, db, callback) => {
    const sqlLogin = `
    ...
    try {
        ...
        if (resultLogin.rows[0]) {
            const resultUserDetail = await db.query(sqlUserDetail, [id, email, phone_num, first_name, last_name, admin]);
